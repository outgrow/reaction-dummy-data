...
export default async function loadProductsAndTags(context, input) {
  ...
  for (existingProductCount; existingProductCount < desiredProductCount; existingProductCount += 1) {
    ...
    const insertedProduct = await context.mutations.createProduct(context, {
      product,
      shopId,
    ...
    context.mutations.createProductVariant(context, {
      variant: {
      ...
      shopId
    }).then((insertedVariant) => context.mutations.updateProductVariantPrices(context, {
      prices: {
        compareAtPrice: 0,
